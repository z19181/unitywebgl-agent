...
function buildIndex() {
  ...
  // 2. 扫描 agents/ (只索引 SOUL.md 和 IMPLEMENTATION.md)
  console.log('[Build Index] Scanning agents/...');
  const agentFiles = scanDirectory(path.join(PROJECT_ROOT, 'agents'), ['.md'])
    .filter(filePath => filePath.endsWith('SOUL.md') || filePath.endsWith('IMPLEMENTATION.md'));
  allFiles.push(...agentFiles);
  console.log(`[Build Index] Found ${agentFiles.length} agent files (SOUL.md + IMPLEMENTATION.md)`);
